...
class FontDrawable(context: Context, faIconRes: Int, isSolid: Boolean, isBrand: Boolean) :
    ...
    private fun init(context: Context, faIconRes: Int, isSolid: Boolean, isBrand: Boolean) {
        ...
        val tf: Typeface? = when {
            isSolid -> FontCache[context, FontCache.FA_FONT_SOLID]
            isBrand -> FontCache[context, FontCache.FA_FONT_BRANDS]
            else -> FontCache[context, FontCache.FA_FONT_REGULAR]
        }
    ...
    fun setTypeface(tf: Typeface?, style: Int) {
        ...
        if (style > 0) {
            tf1 = if (tf1 == null) {
                ...
                Typeface.create(tf1, style)
            }

            typeface = tf1
            // now compute what (if any) algorithmic styling is needed
            val typefaceStyle = tf1?.style ?: 0
    ...
    private fun measureContent() {
        ...
        if (mTextPath != null) {
            ...
            //Measure text bounds
            val desired = ceil(Layout.getDesiredWidth(text, mTextPaint).toDouble())
